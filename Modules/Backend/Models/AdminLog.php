...
class AdminLog extends Model
{
    ...
    {
        $list = AdminLog::select(['log_id', 'admin_name', 'admin_id', 'operate_target', 'operate_ip', 'operate_time', 'operate_status', 'remark'])->Search($params)->orderBy('log_id', 'desc')->paginate($params['limit'])->toArray();
    ...
    public function scopeSearch($query, $keyword){
        return $query->where(function($query) use($keyword) {
            ...
            if (isset($keyword['keyword']) && $keyword['keyword']) {
                $query->where('admin_name', 'like', '%' . strip_tags($keyword['keyword']) . '%');
            }
        });
    }
