...
class Request
{
    ...
    {
        try {
            if(in_array($request->path(),\Config::get('rewrite.dingo.exclude'),true)){
                $request->headers->set('Accept', \Config::get('rewrite.accept'));
                $request->server->set('HTTP_ACCEPT', \Config::get('rewrite.accept'));
            }
