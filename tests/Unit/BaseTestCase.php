...
//            $this->artisan('db:seed');
    ...
    public function apiTest($params)
    {
        ...
        $code = isset($params['code']) ? $params['code'] : 1;
        $headers = empty($params['user']) ? $headers : static::$headers[$params['user']];
        if (strtolower($method) == 'get') {
            return $this->getApiTest($params['uri'], $params['params'], $headers)->assertJson(['code' => $code]);
        } else if (strtolower($method) == 'post') {
            return $this->postApiTest($params['uri'], $params['params'], $headers)->assertJson(['code' => $code]);
        } else {
            return $this->json($method, $this->initUrl($params['uri']), $params['params'], $headers)
                ->assertJson(['code' => $code]);
        }
    }
}